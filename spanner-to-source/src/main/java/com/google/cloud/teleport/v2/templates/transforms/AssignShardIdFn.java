...
public class AssignShardIdFn
  ...
  public AssignShardIdFn(SpannerConfig spannerConfig, Schema schema, Ddl ddl, Long numShards) {
  ...
  public void processElement(ProcessContext c) {
    try {
      TrimmedDataChangeRecord record = new TrimmedDataChangeRecord(c.element());
      ...
      String shardIdColumn = "UserId";
      Long pkValue = keysJson.get(shardIdColumn).asLong();
      record.setUserId(pkValue);
      Long shardId = pkValue % numShards;
      c.output(KV.of(Long.toString(shardId), record));
    } catch (Exception e) {
